...
# ========== HTML TEMPLATES - ĐẦY ĐỦ ANIMATION ==========
INDEX_HTML = """
<!DOCTYPE html>
<html lang="vi">
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'Inter',sans-serif;background:linear-gradient(135deg,#0a0a0a 0%,#0f0f1a 50%,#0a0a0a 100%);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;position:relative;overflow-x:hidden}
        .star{position:fixed;width:2px;height:2px;background:#fff;border-radius:50%;opacity:0;animation:shootingStar 4s linear infinite;z-index:1}
        @keyframes shootingStar{0%{transform:translateX(0)translateY(0);opacity:0}10%{opacity:1}20%{opacity:1}30%{opacity:0}100%{transform:translateX(-200px)translateY(200px);opacity:0}}
        .magic-particle{position:fixed;width:4px;height:4px;background:linear-gradient(135deg,#b000ff,#ff44ff);border-radius:50%;opacity:0;animation:floatMagic 6s infinite;z-index:1}
        @keyframes floatMagic{0%{transform:translateY(100vh) rotate(0deg);opacity:0}20%{opacity:0.8}80%{opacity:0.6}100%{transform:translateY(-100px) rotate(360deg);opacity:0}}
        ...
        .hero{text-align:center;max-width:650px;animation:fadeUp 0.8s;z-index:2}@keyframes fadeUp{from{opacity:0;transform:translateY(50px)}to{opacity:1;transform:translateY(0)}}
        ...
        .btn{background:linear-gradient(135deg,#b000ff,#ff44ff);border:none;padding:16px 45px;font-size:16px;font-weight:600;color:#fff;border-radius:60px;display:inline-flex;align-items:center;gap:10px;text-decoration:none;box-shadow:0 5px 20px rgba(176,0,255,0.4);transition:0.3s;position:relative;overflow:hidden}
        ...
        .loading-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:#0a0a0a;z-index:9999;display:flex;align-items:center;justify-content:center;animation:fadeOut 1s ease 1s forwards}@keyframes fadeOut{to{opacity:0;visibility:hidden}}
        .loader{width:50px;height:50px;border:3px solid rgba(176,0,255,0.3);border-top-color:#b000ff;border-radius:50%;animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
        @media (max-width:768px){h1{font-size:2.8rem}.stats{gap:2rem}.stat-number{font-size:1.5rem}.btn{padding:0.8rem 2rem;font-size:0.9rem}}
        @media (max-width:480px){h1{font-size:2rem}.stats{gap:1rem}.stat-number{font-size:1.2rem}.badge{font-size:0.7rem;padding:0.4rem 1.2rem}}
    </style>
</head>
<body>
    <div class="loading-overlay" id="loadingOverlay"><div class="loader"></div></div>
    <div id="g1" class="glow" style="top:-150px;left:-150px"></div>
    <div id="g2" class="glow" style="bottom:-150px;right:-150px"></div>
    <div class="hero">
    ...
    <script>
        for(let i=0;i<50;i++){let s=document.createElement('div');s.className='star';s.style.top=Math.random()*100+'%';s.style.left=Math.random()*100+'%';s.style.animationDelay=Math.random()*8+'s';document.body.appendChild(s)}
        for(let i=0;i<60;i++){let p=document.createElement('div');p.className='magic-particle';p.style.left=Math.random()*100+'%';p.style.animationDelay=Math.random()*8+'s';document.body.appendChild(p)}
        document.addEventListener('mousemove',function(e){let g1=document.getElementById('g1');let g2=document.getElementById('g2');if(g1)g1.style.transform=`translate(${e.clientX*0.05}px,${e.clientY*0.05}px)`;if(g2)g2.style.transform=`translate(${-e.clientX*0.03}px,${-e.clientY*0.03}px)`});
        setTimeout(()=>{let ld=document.getElementById('loadingOverlay');if(ld)ld.style.display='none'},1500);
    </script>
</body>
</html>
"""
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
    <title>Bước {{ step }} - DRAGON PINGX PREMIUM</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'Inter',sans-serif;background:linear-gradient(135deg,#0a0a0a,#0f0f1a);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;position:relative}
        .bg-effect{position:fixed;top:0;left:0;width:100%;height:100%;overflow:hidden;z-index:0}
        .bg-circle{position:absolute;border-radius:50%;background:rgba(176,0,255,0.05);animation:float 20s infinite}
        @keyframes float{0%,100%{transform:translateY(0) rotate(0deg)}50%{transform:translateY(-50px) rotate(180deg)}}
        .card{position:relative;z-index:2;max-width:550px;width:100%;background:rgba(15,23,42,0.95);backdrop-filter:blur(20px);border-radius:32px;padding:32px;border:1px solid rgba(176,0,255,0.3);text-align:center;animation:fadeScale 0.6s}
        @keyframes fadeScale{from{opacity:0;transform:scale(0.9)}to{opacity:1;transform:scale(1)}}
        .step-icon{width:80px;height:80px;background:linear-gradient(135deg,#b000ff,#ff44ff);border-radius:2rem;display:flex;align-items:center;justify-content:center;margin:0 auto 1.5rem;animation:rotate3D 4s infinite}
        @keyframes rotate3D{0%{transform:rotateY(0deg)}50%{transform:rotateY(180deg)}100%{transform:rotateY(360deg)}}
        ...
        .task-link{background:rgba(0,0,0,0.4);border-radius:16px;padding:16px;margin:20px 0;word-break:break-all;border:1px dashed rgba(176,0,255,0.3);transition:0.3s}
        .task-link:hover{border-color:#b000ff;background:rgba(176,0,255,0.05)}
        .task-link a{color:#b000ff;text-decoration:none;font-size:14px}
        .btn-group{display:flex;gap:16px;margin-top:24px}
        .btn-continue{flex:1;background:linear-gradient(135deg,#00cc66,#00ff88);border:none;padding:14px;border-radius:16px;color:#fff;font-weight:600;cursor:pointer;transition:0.3s;position:relative;overflow:hidden}
        .btn-continue::before{content:'';position:absolute;top:50%;left:50%;width:0;height:0;border-radius:50%;background:rgba(255,255,255,0.3);transform:translate(-50%,-50%);transition:width 0.6s,height 0.6s}
        .btn-continue:hover::before{width:300px;height:300px}
        .btn-continue:hover{transform:translateY(-2px);box-shadow:0 10px 20px rgba(0,255,136,0.3)}
        ...
        .loading{display:inline-block;width:18px;height:18px;border:2px solid rgba(255,255,255,0.3);border-top-color:#fff;border-radius:50%;animation:spin 0.8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
        .warning{font-size:13px;color:#f87171;margin-top:16px;padding:10px;background:rgba(239,68,68,0.1);border-radius:10px;display:none}
        .warning.show{display:block;animation:shake 0.5s}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-5px)}75%{transform:translateX(5px)}}
    </style>
</head>
<body>
    <div class="bg-effect" id="bgEffect"></div>
    <div class="card">
        <div class="step-icon"><svg width="40" height="40" fill="none" stroke="white" viewBox="0 0 24 24">{% if step == 1 %}<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>{% elif step == 2 %}<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>{% else %}<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/>{% endif %}</svg></div>
        <div class="step-badge">📌 BƯỚC {{ step }}/3</div>
        <h2>{{ title }}</h2>
        <div class="desc">{{ desc }}</div>
        <div class="info">💰 Hoàn thành nhiệm vụ để nhận KEY MIỄN PHÍ!</div>
        <div class="task-link"><div style="font-size:12px;color:#666;margin-bottom:8px;">🔗 Link nhiệm vụ của bạn:</div><a href="{{ url }}" target="_blank" id="taskLink">{{ url }}</a></div>
        <div class="btn-group"><a href="{{ back_url }}" class="btn-back">🔙 Quay lại</a><button class="btn-continue" onclick="check()" id="continueBtn">✅ TIẾP TỤC</button></div>
        <div class="warning" id="warningMsg">⚠️ Bạn chưa hoàn thành nhiệm vụ!</div>
    </div>
    <script>
        for(let i=0;i<15;i++){let c=document.createElement('div');c.className='bg-circle';let s=100+Math.random()*200;c.style.width=s+'px';c.style.height=s+'px';c.style.left=Math.random()*100+'%';c.style.top=Math.random()*100+'%';c.style.animationDelay=Math.random()*20+'s';document.getElementById('bgEffect').appendChild(c)}
        let sid="{{ sid }}",step={{ step }},checking=false;
        async function check(){if(checking)return;checking=true;const btn=document.getElementById('continueBtn'),original=btn.innerHTML;btn.innerHTML='<span class="loading"></span> Đang kiểm tra...';btn.disabled=true;document.getElementById('warningMsg').classList.remove('show');try{const res=await fetch(`/api/check/${sid}/${step}`),data=await res.json();if(data.completed){window.location.href=data.next}else{document.getElementById('warningMsg').classList.add('show');btn.innerHTML=original;btn.disabled=false;checking=false;window.open(document.getElementById('taskLink').href,'_blank')}}catch(e){document.getElementById('warningMsg').innerHTML='⚠️ Lỗi, thử lại!';document.getElementById('warningMsg').classList.add('show');btn.innerHTML=original;btn.disabled=false;checking=false}}
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .sparkle{position:fixed;width:100%;height:100%;pointer-events:none;z-index:9998}
        .sparkle-star{position:absolute;font-size:20px;animation:sparkleFade 1s forwards}@keyframes sparkleFade{0%{opacity:1;transform:scale(1)}100%{opacity:0;transform:scale(1.5)}}
        .card{max-width:520px;width:100%;background:rgba(15,23,42,0.95);backdrop-filter:blur(20px);border-radius:32px;padding:40px;text-align:center;border:1px solid rgba(176,0,255,0.4);animation:bounce 0.8s;z-index:2}
        ...
        .key-box{background:linear-gradient(135deg,#0f172a,#1a1a2e);border-radius:20px;padding:24px;margin:24px 0;border:1px dashed #b000ff;transition:0.3s}
        .key-box:hover{border-color:#ff44ff;box-shadow:0 0 20px rgba(176,0,255,0.3)}
        .key-value{font-family:monospace;font-size:20px;font-weight:700;background:linear-gradient(135deg,#b000ff,#ff44ff);background-clip:text;-webkit-background-clip:text;color:transparent;word-break:break-all;margin:12px 0;cursor:pointer;padding:10px;border-radius:10px}
        .key-value:hover{background:rgba(176,0,255,0.2);transform:scale(1.02)}
        .copy-btn{background:linear-gradient(135deg,#b000ff,#ff44ff);border:none;padding:12px 32px;border-radius:40px;color:#fff;cursor:pointer;font-weight:600;transition:0.3s;position:relative;overflow:hidden}
        .copy-btn::before{content:'';position:absolute;top:50%;left:50%;width:0;height:0;border-radius:50%;background:rgba(255,255,255,0.3);transform:translate(-50%,-50%);transition:width 0.6s,height 0.6s}
        .copy-btn:hover::before{width:300px;height:300px}
        .copy-btn:hover{transform:translateY(-3px);box-shadow:0 10px 20px rgba(176,0,255,0.4)}
        .btn-back{display:inline-block;background:rgba(176,0,255,0.2);text-decoration:none;color:#b000ff;padding:10px 24px;border-radius:40px;margin-top:16px;transition:0.3s}
        .btn-back:hover{background:rgba(176,0,255,0.4)}
...
<body>
    ...
    <div class="card">
        ...
        <div class="key-box"><div style="font-size:11px;color:#b000ff;letter-spacing:2px;margin-bottom:10px">🔑 KEY KÍCH HOẠT</div><div class="key-value" id="licenseKey" onclick="copyKey()">{{ key }}</div><button class="copy-btn" onclick="copyKey()">📋 Sao chép key</button></div>
    ...
    <script>
        ...
        function createConfetti(){for(let i=0;i<150;i++){let c=document.createElement('div');c.className='confetti';c.style.left=Math.random()*100+'%';c.style.top='-10px';c.style.animationDelay=Math.random()*2+'s';c.style.animationDuration=(2+Math.random()*3)+'s';c.style.backgroundColor=colors[Math.floor(Math.random()*colors.length)];c.style.width=(5+Math.random()*10)+'px';c.style.height=(5+Math.random()*10)+'px';document.body.appendChild(c);setTimeout(()=>c.remove(),5000)}}
        function createSparkle(){const s=document.getElementById('sparkle');for(let i=0;i<50;i++){let d=document.createElement('div');d.innerHTML=['✨','⭐','🌟','💫','⚡'][Math.floor(Math.random()*5)];d.className='sparkle-star';d.style.left=Math.random()*100+'%';d.style.top=Math.random()*100+'%';d.style.fontSize=(15+Math.random()*25)+'px';d.style.opacity=Math.random();d.style.animationDelay=Math.random()*1+'s';s.appendChild(d);setTimeout(()=>d.remove(),1000)}}
        createConfetti();createSparkle();setTimeout(createConfetti,1000);setTimeout(createSparkle,1500);
